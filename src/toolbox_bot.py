# ==============================================================================
#  MAHDI'S TOOLBOX BOT - All-In-One Solution
# ==============================================================================
import os
import requests
import random
...
# --- إعدادات أساسية ---
TOKEN = "1936058114:AAHm19u1R6lv_vShGio-MIo4Z0rjVUoew_U" # توكن البوت الأساسي
CHAT_ID = "1148797883" # الـ ID الخاص بك

# --- متغيرات الحالة العامة ---
current_task = "idle" # "idle", "hunting", "rushing"
is_running = False
hits = 0
fails = 0
...
def rusher_worker(target_username):
    ...
    while is_running:
        ...
        try:
            ...
            security_nonce = soup.find('input', {'name': 'security'})['value']
            
            if not security_nonce:
                fails += 1; continue

            random_email = f"user{random.randint(1000, 99999)}@example.com"
            payload = {'action': 'get_free_followers', 'username': target_username, 'email': random_email, 'security': security_nonce}
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_running = False
    current_task = "idle"
    await update.message.reply_text("🛑 Stopping current task... Please wait for workers to finish.")
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('next_step') == 'awaiting_rush_target':
        ...
        thread.start()
        context.user_data['next_step'] = None # Clear state
